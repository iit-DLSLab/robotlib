...
namespace dummy_hexapod
{
	...
	class DummyHexapod : public Robot<NLEGS, NLINKS_TOT, NJOINTS_TOT>
	{
	public:
		DummyHexapod(Trunk& trunk, std::array<LimbBase, NLEGS>& limbs)
			: Robot<NLEGS, NLINKS_TOT, NJOINTS_TOT>("Hexapod", trunk, limbs)
		{
		...
		virtual Eigen::Vector3d getFootPosition(const JointState& q, const robotlib::Frame& foot) const override
		{
...
robotlib::dummy_hexapod::DummyLeg makeLeg(const std::string& legName, robotlib::Link& trunk)
{
	...
	return robotlib::dummy_hexapod::DummyLeg(legName, {assembly, upperleg, lowerleg}, {haa, hfe, kfe});
}
...
extern "C" std::shared_ptr<robotlib::RobotBase> createRobot_t()
{
	...
	std::array<robotlib::LimbBase, robotlib::dummy_hexapod::NLEGS> 
		limbs({makeLeg("LF", trunk), makeLeg("RF", trunk), makeLeg("LH", trunk), makeLeg("RH", trunk), makeLeg("LC", trunk), makeLeg("RC", trunk)});
	
	return std::make_shared<robotlib::dummy_hexapod::DummyHexapod>(trunk, limbs);
}
