...
#include "robotlib/rbd/robcogen_commons.h"

using namespace dls::rbd;
using namespace dls::dog;

// Initialization of static-const data
const dls::dog::HyQReal::dyn::ForwardDynamics::ExtForces
    dls::dog::HyQReal::dyn::ForwardDynamics::zeroExtForces(Force::Zero());

dls::dog::HyQReal::dyn::ForwardDynamics::ForwardDynamics(InertiaProperties& inertia, MotionTransforms& transforms) :
...
void dls::dog::HyQReal::dyn::ForwardDynamics::fd(
    JointState& qdd,
    ...
    const ExtForces& fext/* = zeroExtForces */)
{
    ...
    LF_upperleg_v(AZ) += qd(LF_HFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(LF_upperleg_v, vcross);
    LF_upperleg_c = vcross.col(AZ) * qd(LF_HFE);
    ...
    LF_lowerleg_v(AZ) += qd(LF_KFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(LF_lowerleg_v, vcross);
    LF_lowerleg_c = vcross.col(AZ) * qd(LF_KFE);
    ...
    RF_hipassembly_v(AZ) += qd(RF_HAA);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RF_hipassembly_v, vcross);
    RF_hipassembly_c = vcross.col(AZ) * qd(RF_HAA);
    ...
    RF_upperleg_v(AZ) += qd(RF_HFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RF_upperleg_v, vcross);
    RF_upperleg_c = vcross.col(AZ) * qd(RF_HFE);
    ...
    RF_lowerleg_v(AZ) += qd(RF_KFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RF_lowerleg_v, vcross);
    RF_lowerleg_c = vcross.col(AZ) * qd(RF_KFE);
    ...
    LH_hipassembly_v(AZ) += qd(LH_HAA);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(LH_hipassembly_v, vcross);
    LH_hipassembly_c = vcross.col(AZ) * qd(LH_HAA);

    //  - The bias force term:
    LH_hipassembly_p += vxIv(LH_hipassembly_v, LH_hipassembly_AI);

    // + Link LH_upperleg
    //  - The spatial velocity:
    LH_upperleg_v = (motionTransforms-> fr_LH_upperleg_X_fr_LH_hipassembly) * LH_hipassembly_v;
    LH_upperleg_v(AZ) += qd(LH_HFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(LH_upperleg_v, vcross);
    LH_upperleg_c = vcross.col(AZ) * qd(LH_HFE);

    //  - The bias force term:
    LH_upperleg_p += vxIv(LH_upperleg_v, LH_upperleg_AI);

    // + Link LH_lowerleg
    //  - The spatial velocity:
    LH_lowerleg_v = (motionTransforms-> fr_LH_lowerleg_X_fr_LH_upperleg) * LH_upperleg_v;
    LH_lowerleg_v(AZ) += qd(LH_KFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(LH_lowerleg_v, vcross);
    LH_lowerleg_c = vcross.col(AZ) * qd(LH_KFE);

    //  - The bias force term:
    LH_lowerleg_p += vxIv(LH_lowerleg_v, LH_lowerleg_AI);

    // + Link RH_hipassembly
    //  - The spatial velocity:
    RH_hipassembly_v = (motionTransforms-> fr_RH_hipassembly_X_fr_trunk) * trunk_v;
    RH_hipassembly_v(AZ) += qd(RH_HAA);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RH_hipassembly_v, vcross);
    RH_hipassembly_c = vcross.col(AZ) * qd(RH_HAA);

    //  - The bias force term:
    RH_hipassembly_p += vxIv(RH_hipassembly_v, RH_hipassembly_AI);

    // + Link RH_upperleg
    //  - The spatial velocity:
    RH_upperleg_v = (motionTransforms-> fr_RH_upperleg_X_fr_RH_hipassembly) * RH_hipassembly_v;
    RH_upperleg_v(AZ) += qd(RH_HFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RH_upperleg_v, vcross);
    RH_upperleg_c = vcross.col(AZ) * qd(RH_HFE);

    //  - The bias force term:
    RH_upperleg_p += vxIv(RH_upperleg_v, RH_upperleg_AI);

    // + Link RH_lowerleg
    //  - The spatial velocity:
    RH_lowerleg_v = (motionTransforms-> fr_RH_lowerleg_X_fr_RH_upperleg) * RH_upperleg_v;
    RH_lowerleg_v(AZ) += qd(RH_KFE);

    //  - The velocity-product acceleration term:
    motionCrossProductMx(RH_lowerleg_v, vcross);
    RH_lowerleg_c = vcross.col(AZ) * qd(RH_KFE);

    //  - The bias force term:
    RH_lowerleg_p += vxIv(RH_lowerleg_v, RH_lowerleg_AI);
    ...
    // + Link RH_lowerleg
    RH_lowerleg_u = tau(RH_KFE) - RH_lowerleg_p(AZ);
    RH_lowerleg_U = RH_lowerleg_AI.col(AZ);
    RH_lowerleg_D = RH_lowerleg_U(AZ);

    compute_Ia_revolute(RH_lowerleg_AI, RH_lowerleg_U, RH_lowerleg_D, Ia_r);  // same as: Ia_r = RH_lowerleg_AI - RH_lowerleg_U/RH_lowerleg_D * RH_lowerleg_U.transpose();
    ...
    // + Link RH_upperleg
    RH_upperleg_u = tau(RH_HFE) - RH_upperleg_p(AZ);
    RH_upperleg_U = RH_upperleg_AI.col(AZ);
    RH_upperleg_D = RH_upperleg_U(AZ);

    compute_Ia_revolute(RH_upperleg_AI, RH_upperleg_U, RH_upperleg_D, Ia_r);  // same as: Ia_r = RH_upperleg_AI - RH_upperleg_U/RH_upperleg_D * RH_upperleg_U.transpose();
    ...
    // + Link RH_hipassembly
    RH_hipassembly_u = tau(RH_HAA) - RH_hipassembly_p(AZ);
    RH_hipassembly_U = RH_hipassembly_AI.col(AZ);
    RH_hipassembly_D = RH_hipassembly_U(AZ);

    compute_Ia_revolute(RH_hipassembly_AI, RH_hipassembly_U, RH_hipassembly_D, Ia_r);  // same as: Ia_r = RH_hipassembly_AI - RH_hipassembly_U/RH_hipassembly_D * RH_hipassembly_U.transpose();
    ...
    // + Link LH_lowerleg
    LH_lowerleg_u = tau(LH_KFE) - LH_lowerleg_p(AZ);
    LH_lowerleg_U = LH_lowerleg_AI.col(AZ);
    LH_lowerleg_D = LH_lowerleg_U(AZ);

    compute_Ia_revolute(LH_lowerleg_AI, LH_lowerleg_U, LH_lowerleg_D, Ia_r);  // same as: Ia_r = LH_lowerleg_AI - LH_lowerleg_U/LH_lowerleg_D * LH_lowerleg_U.transpose();
    ...
    // + Link LH_upperleg
    LH_upperleg_u = tau(LH_HFE) - LH_upperleg_p(AZ);
    LH_upperleg_U = LH_upperleg_AI.col(AZ);
    LH_upperleg_D = LH_upperleg_U(AZ);

    compute_Ia_revolute(LH_upperleg_AI, LH_upperleg_U, LH_upperleg_D, Ia_r);  // same as: Ia_r = LH_upperleg_AI - LH_upperleg_U/LH_upperleg_D * LH_upperleg_U.transpose();
    ...
    // + Link LH_hipassembly
    LH_hipassembly_u = tau(LH_HAA) - LH_hipassembly_p(AZ);
    LH_hipassembly_U = LH_hipassembly_AI.col(AZ);
    LH_hipassembly_D = LH_hipassembly_U(AZ);

    compute_Ia_revolute(LH_hipassembly_AI, LH_hipassembly_U, LH_hipassembly_D, Ia_r);  // same as: Ia_r = LH_hipassembly_AI - LH_hipassembly_U/LH_hipassembly_D * LH_hipassembly_U.transpose();
    ...
    // + Link RF_lowerleg
    RF_lowerleg_u = tau(RF_KFE) - RF_lowerleg_p(AZ);
    ...
    // + Link RF_upperleg
    RF_upperleg_u = tau(RF_HFE) - RF_upperleg_p(AZ);
    ...
    // + Link RF_hipassembly
    RF_hipassembly_u = tau(RF_HAA) - RF_hipassembly_p(AZ);
    ...
    // + Link LF_lowerleg
    LF_lowerleg_u = tau(LF_KFE) - LF_lowerleg_p(AZ);
    ...
    // + Link LF_upperleg
    LF_upperleg_u = tau(LF_HFE) - LF_upperleg_p(AZ);
    ...
    // + Link LF_hipassembly
    LF_hipassembly_u = tau(LF_HAA) - LF_hipassembly_p(AZ);
    ...
    LF_hipassembly_a(AZ) += qdd(LF_HAA);
    ...
    RF_hipassembly_a(AZ) += qdd(RF_HAA);
    ...
    LH_hipassembly_a(AZ) += qdd(LH_HAA);
    ...
    RH_hipassembly_a(AZ) += qdd(RH_HAA);
