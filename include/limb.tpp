...
namespace dls
{
    namespace robotlib
    {
        ...
        Limb<NJOINTS, NLINKS>::Limb(const std::string &name, const std::array<std::shared_ptr<Joint>, NJOINTS> joints, const std::array<std::shared_ptr<Link>, NLINKS> links)
            : LimbBase(name), joints_(std::make_shared<Container<Joint, NJOINTS>>(joints)), links_(links){};

        template <unsigned int NJOINTS, unsigned int NLINKS>
        Limb<NJOINTS, NLINKS>::~Limb(){};

        template <unsigned int NJOINTS, unsigned int NLINKS>
        std::shared_ptr<Frame> Limb<NJOINTS, NLINKS>::getLink(const int linkId) { return links_[linkId]; };
        template <unsigned int NJOINTS, unsigned int NLINKS>
        const std::shared_ptr<Frame> Limb<NJOINTS, NLINKS>::getJoint(const int jointId) const { return (*joints_)[jointId]; };
