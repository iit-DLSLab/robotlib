...
*******************************************************************************/
#ifndef _ROBOTLIB_BASE_INVERSE_DYNAMICS_HPP_
#define _ROBOTLIB_BASE_INVERSE_DYNAMICS_HPP_
...
public:
	...
	typedef LinkDataMap<dls::rbd::ForceVector> ExtForces;
...
public:
	...
	(
		...
		const ExtForces& fext = LinkDataMap<dls::rbd::ForceVector>(Force::Zero())
	) = 0;
	 ...
	 */ ///@{
	virtual void id_fully_actuated
	(
		Force& baseWrench,
		JointState& jForces,
		const Acceleration& g,
	 ...
	 */
	///@{
	virtual void G_terms_fully_actuated
	(
		Force& baseWrench,
		JointState& jForces,
		const Acceleration& g,
	 ...
	 */
	///@{
	virtual void C_terms_fully_actuated
	(
		Force& baseWrench,
		JointState& jForces,
		const Velocity& trunk_v,
	...
	virtual Velocity getLinkVelocity
	(
		const JointState& q,
		const JointState& qd,
		const JointState& qdd,
	...
	virtual Acceleration getLinkAcceleration
	(
		const JointState& q,
		const JointState& qd,
		const JointState& qdd,
