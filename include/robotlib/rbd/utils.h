 ...
 */
/* } CPYHDR */
#ifndef _ROBOTLIB_RBD_UTILS_HPP_
#define _ROBOTLIB_RBD_UTILS_HPP_
...
public:
    ...
    static void randomGravity(VelocityVector& g)
    {
        ...
        linearPart(g) = (linearPart(g) / linearPart(g).norm()) * dls::rbd::g;
    }

    template <class RT>
    static void cmdlineargs_spatialv(int argc, char** argv,
            dls::rbd::Vector6D& vec)
    {
        if(argc < 6) {
...
#endif // _ROBOTLIB_RBD_UTILS_HPP_