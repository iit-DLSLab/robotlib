...
#ifndef _ROBOTLIB_RBD_ROBCOGEN_COMMONS_HPP_
#define _ROBOTLIB_RBD_ROBCOGEN_COMMONS_HPP_


#include "robotlib/rbd/rbd.h"
#include "robotlib/rbd/InertiaMatrix.h"
#include "robotlib/rbd/TransformsBase.h"
#include "robotlib/rbd/internals.h"
 ...
 *
 * The functions in this module implement some specific expressions or
 * functionality recurring in the code generated by RobCoGen.
 *
 * The implementation is usually a bit more efficient than the straightforward
 * way of doing the same thing, with matrix algebra.
 *
 * This functions were specifically written to be used in the source files
...
ForceVector vxIv(const VelocityVector& v, const MatrixBase<Derived>& I)
{
    ...
    double wx2 = v(AX)*v(AX);
    double wy2 = v(AY)*v(AY);
    double wz2 = v(AZ)*v(AZ);

    double v_AXAY = v(AX)*v(AY);
    double v_AXAZ = v(AX)*v(AZ);
    double v_AXLY = v(AX)*v(LY);
    double v_AXLZ = v(AX)*v(LZ);

    double v_AYAZ = v(AY)*v(AZ);
    double v_AYLX = v(AY)*v(LX);
    double v_AYLZ = v(AY)*v(LZ);

    double v_AZLX = v(AZ)*v(LX);
    double v_AZLY = v(AZ)*v(LY);

    double m   = I(LX,LX); // the mass
    ...
    internal::SymmMat3x3Coefficients Iaa(
 ...
 */
template <typename Derived>
inline ForceVector vxIv(const double& omegaz, const MatrixBase<Derived>& I)
{
    double wz2 = omegaz*omegaz;
    ForceVector ret;
    ret(AX) = -I(AY,AZ) * wz2;
    ret(AY) =  I(AX,AY) * wz2;
    ret(AZ) =  0;
 ...
 */
inline void motionCrossProductMx(const VelocityVector& v, Matrix66d& vx)
{
 ...
 *     zero elements of the cross product matrix).
 */
inline void forceCrossProductMx(const VelocityVector& v, Matrix66d& vx)
{
    vx(AY,AZ) = vx(LY,LZ) = - ( vx(AZ,AY) = vx(LZ,LY) = v(AX) );
    vx(AZ,AX) = vx(LZ,LX) = - ( vx(AX,AZ) = vx(LX,LZ) = v(AY) );
    vx(AX,AY) = vx(LX,LY) = - ( vx(AY,AX) = vx(LY,LX) = v(AZ) );
        ...
        const MatrixBase<D3>& Ia_B_const)
{
    ...
    double rx = E.XY * XM(LZ,AX) + E.YY * XM(LZ,AY) + E.ZY * XM(LZ,AZ);
    double ry = E.XZ * XM(LX,AX) + E.YZ * XM(LX,AY) + E.ZZ * XM(LX,AZ);
    double rz = E.XX * XM(LY,AX) + E.YX * XM(LY,AY) + E.ZX * XM(LY,AZ);

    // Angular-angular 3x3 sub-block:

    internal::SymmMat3x3Coefficients aux1, aux2;
    ...
    const internal::SymmMat3x3Coefficients M(
            Ia_A(LX,LX),Ia_A(LX,LY),Ia_A(LX,LZ)
                       ,Ia_A(LY,LY),Ia_A(LY,LZ)
                                   ,Ia_A(LZ,LZ));
    internal::Mat3x3Coefficients rxM;
    ...
    // ## compute  E ( .. ) E^T
    internal::rot_symmetric_EAET(E, aux1, aux2);
    ...
    internal::Mat3x3Coefficients aux3;
    internal::rot_EAET(E, rxM, aux3);
    ...
    // Linear-linear block
    internal::rot_symmetric_EAET(E, M, aux1);
    Ia_B(LX,LX) = aux1.XX;
    Ia_B(LY,LY) = aux1.YY;
    Ia_B(LZ,LZ) = aux1.ZZ;
        ...
        const MatrixBase<D3>& Ia_B_const)
{
    ...
    internal::Mat3x3Coefficients aux3;
    internal::rot_EAET(E, rxM, aux3); // TODO exploit the zeros in rxM
    // copy the result, also to the symmetric 3x3 block
    Ia_B(LX,AX) = Ia_B(AX,LX) = aux3.XX;
    Ia_B(LY,AX) = Ia_B(AX,LY) = aux3.XY;
    Ia_B(LZ,AX) = Ia_B(AX,LZ) = aux3.XZ;
    ...
    // Linear-linear block
    internal::rot_symmetric_EAET(E, M, aux1); // TODO exploit the zeros in M
    Ia_B(LX,LX) = aux1.XX;
    Ia_B(LY,LY) = aux1.YY;
    Ia_B(LZ,LZ) = aux1.ZZ;
    Ia_B(LY,LX) = Ia_B(LX,LY) = aux1.XY;
...
#endif // _ROBOTLIB_RBD_ROBCOGEN_COMMONS_HPP_
